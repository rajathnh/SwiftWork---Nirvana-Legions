...
async function getFreelancerData(freelancerId) {
  try {
    const response = await fetch(
      `http://localhost:5000/api/v1/freelancer/${freelancerId}`
    );
    if (!response.ok) {
    ...
    alert(
      "An error occurred while fetching freelancer data. Please try again later."
    );
  }
}
...
async function getFreelancerProposals(freelancerId) {
  try {
    const response = await fetch(
      `http://localhost:5000/api/v1/freelancer/${freelancerId}/proposals`
    );
    if (!response.ok) {
...
async function getFreelancerBadges(freelancerId) {
  try {
    const response = await fetch(
      `http://localhost:5000/api/v1/test/freelancer-badges/${freelancerId}`
    );
    if (!response.ok) {
    ...
    <div class="mx-auto p-6">
        <div class="flex flex-col lg:flex-row lg:space-x-8">
            <div class="profile-header border border-slate-300 hover:border-slate-400 flex flex-col items-center lg:w-1/3 p-6 rounded-lg shadow-sm mb-6 lg:mb-0">
                ...
                <h2 class="text-2xl font-bold mt-6 text-center text-white">${
                  freelancer.name || "No Name Provided"
                }</h2>
                <p class="text-center text-white mt-4"><strong>Email:</strong> ${
               ...
               <div class="w-full">
                    <p class="text-center text-white mt-4">
                        ...
                        ${
                          freelancer.skills && freelancer.skills.length
        ...
        <div class="reviews bg-white p-6 border border-slate-300 hover:border-slate-400 rounded-lg shadow-sm mt-6 lg:mt-8">
            ...
            <p class="text-gray-700"><strong>Number of Reviews:</strong> ${
              freelancer.numOfReviews || 0
            }</p>
        </div>
    </div>`;
...
function displayFreelancerProposals(proposals) {
  ...
  const openGigs = proposals.filter(
    (proposal) => proposal.gig?.status === "open"
  );
  const assignedGigs = proposals.filter(
    (proposal) =>
      proposal.gig?.status === "assigned" &&
      proposal.freelancer._id === freelancerId
  );
  const approvalPendingGigs = proposals.filter(
    (proposal) =>
      proposal.gig?.status === "approval pending" &&
      proposal.freelancer._id === freelancerId
  );
  const assignedToOthersGigs = proposals.filter(
    (proposal) =>
      proposal.gig?.status === "assigned" &&
      proposal.freelancer._id !== freelancerId
  );
  const completedProjects = proposals.filter(
    (proposal) =>
  ...
  displayGigCategory(
    proposalsSection,
    "Approval Pending Projects",
    approvalPendingGigs
  );
  displayGigCategory(
    proposalsSection,
    "Assigned to Others Projects",
    assignedToOthersGigs
  );
  ...
  if (gigs.length > 0) {
    ...
    gigs.forEach((proposal) => {
            ...
            <div class="p-4">
                <div class="proposal-card bg-white p-6 rounded-lg shadow-lg border border-slate-300 hover:border-slate-400 text-left">
                    <a href="${gigLink}" class="proposal-link block">
                        <p class="text-lg font-semibold text-gray-800"><strong>Project:</strong> ${
                          gig.title || "No Title"
                        }</p>
                        <p class="text-gray-700"><strong>Description:</strong> ${
                          gig.description || "No Description"
                        }</p>
                        <p class="text-gray-700"><strong>Budget:</strong> ₹${
                          gig.budget || "N/A"
                        }</p>
                        <p class="text-gray-700"><strong>Deadline:</strong> ${
                          new Date(gig.deadline).toDateString() || "N/A"
                        }</p>
                        <p class="text-gray-700"><strong>Bid Amount:</strong> ₹${
                          proposal.bidAmount || "N/A"
                        }</p>
                        <p class="text-gray-700"><strong>Proposal Message:</strong> ${
                          proposal.proposalMessage || "No Message Provided"
                        }</p>
      ...
      if (categoryTitle === "Assigned Projects") {
        ...
        submitProjectBtn.classList.add(
          "submit-project-btn",
          "bg-green-500",
          "text-white",
          "py-2",
...
const displayNotifications = (notifications) => {
  ...
  notifications.forEach((notification) => {
    ...
    notificationItem.className = notification.seen
      ? "notification seen"
      : "notification";
